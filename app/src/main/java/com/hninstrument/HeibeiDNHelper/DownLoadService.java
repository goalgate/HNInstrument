package com.hninstrument.HeibeiDNHelper;
...
import com.hninstrument.CBSD_HeibeiDNActivity;
...
public class DownLoadService extends Service {
    private static final boolean rootInstall = false;
    ...
    public void startDownload(/*String apkUrl*/) {
        new ServerConnectionUtil().download("http://124.172.232.89:8050/daServer/updateADA.do?ver=" + AppUtils.getAppVersionName() + "&url=" + config.getString("ServerId") + "&daid=" + config.getString("devid"), config.getString("ServerId"), new ServerConnectionUtil.Callback() {
            @Override
            public void onResponse(String response) {
                if (response != null) {
                    if (response.equals("true")) {
                        installAuto(APK_PATH);
                    }
                }
            }
        });
