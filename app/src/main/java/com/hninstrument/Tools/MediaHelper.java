...
public class MediaHelper {
    public enum Text {
        ...
        msg_visit, err_omtk, wait_reupload, waiting, second_err, alarm ,err_connect_ns,
    }
    ...
    public static void play(Text text) {
        try {
            ...
            switch (text) {
                ...
                case err_connect:
                    fileDescriptor = AppInit.getContext().getAssets()
                            .openFd("mp3" + File.separator + "连接服务器失败，离线数据已保存.mp3");
                    play(fileDescriptor);
                    break;
                case err_samePerson:
                    fileDescriptor = AppInit.getContext().getAssets()
                ...
                case second_err:
                    fileDescriptor = AppInit.getContext().getAssets()
                            .openFd("mp3" + File.separator + "仓库门已进入撤防状态，离线数据已保存.mp3");
                    play(fileDescriptor);
                    break;
                case alarm:
                    fileDescriptor = AppInit.getContext().getAssets()
